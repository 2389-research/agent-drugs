...
exports.oauthCallback = exports.oauthCallbackHandler = void 0;
 ...
 */
async function oauthCallbackHandler(request) {
    // Verify user is authenticated
    if (!request.auth || !request.auth.uid) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to authorize OAuth');
    }
    const { clientId, redirectUri, scope, state, codeChallenge, codeChallengeMethod, agentName, existingAgentId } = request.data;
    // Validate PKCE
    if (codeChallengeMethod !== 'S256') {
        throw new functions.https.HttpsError('invalid-argument', 'Only S256 code_challenge_method is supported');
    }
    if (!codeChallenge) {
        throw new functions.https.HttpsError('invalid-argument', 'code_challenge is required');
    }
    ...
    if (existingAgentId) {
        ...
        if (!agentDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Specified agent not found');
        }
        const agentData = agentDoc.data();
        if (agentData?.userId !== uid) {
            throw new functions.https.HttpsError('permission-denied', 'You do not have permission to use this agent');
        }
 ...
 */
exports.oauthCallback = functions.https.onCall(async (request) => {
